...
if len(fontes_prontas) < 2:import streamlit as st
import pandas as pd
import re
import unicodedata
import io
...
def extrair_identificador_nota(n, tipo_doc):
    ...
    Usa apenas o número para NFSe ou quando a chave não estiver completa.
    """
    if pd.isna(n): return ""
    s = str(n).strip()
    digitos = re.sub(r'\D', '', s)
    
    # Se for NF-e/CT-e/NFC-e e tiver 44 dígitos, usa a chave completa
    if len(digitos) == 44 and tipo_doc != "NFSe":
        return digitos
    
    # Para NFSe ou notas sem chave completa, extrai a numeração sequencial
    if len(digitos) == 44:
...
def processar_fonte(df, col_nota, col_data, col_valor, col_evento, usar_data, tipo_doc):
    ...
    # Purga de Status Invalidados
    if col_evento and col_evento != "Nenhuma":
        res['invalida'] = df[col_evento].apply(verificar_status_invalido)
    else:
        res['invalida'] = False

    # Remove notas sem identificador e notas marcadas como Canceladas/Denegadas
    ...
    with cols_layout[i]:
        ...
        if f_upload:
            ...
            if not df_bruto.empty:
                ...
                col_nota = st.selectbox("Chave / Número NF", cols, index=idx_nota, key=f"nota_{codigo}")
                col_data = st.selectbox(
                    "Data" + (" (obrigatória)" if usar_data else " (opcional)"),
                    opcoes_com_nenhuma, index=idx_data + 1, key=f"data_{codigo}"
                )
                col_valor = st.selectbox("Valor Total", cols, index=idx_valor, key=f"valor_{codigo}")
                col_evento = st.selectbox(
                    "Status / Situação (Filtra Canceladas)", opcoes_com_nenhuma,
                    index=(idx_evento + 1) if idx_evento is not None else 0, key=f"evento_{codigo}"
                )
...
if st.button("🚀 Processar Conciliação e Retornar Divergências", type="primary", use_container_width=True):
    with st.spinner("Lendo relatórios, eliminando canceladas e cruzando bases..."):
        try:
            ...
            for i, codigo in enumerate(fontes_prontas):
                ...
                with metricas[i]:
                    st.metric(f"Total Valido — {dados_fontes[codigo]['fonte']}", formatar_moeda_br(total))

            # Identificação Rígida de Divergências
            ...
            if divergencias.empty:
                ...
                # Exportação para trabalho do analista
